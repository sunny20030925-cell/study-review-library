...
- Visual Polish：**2／21 完成**；`advanced-statistics`、`mathematical-economics` 已通過，剩餘 19 本在 queue。
  - 《高等統計學》完成《高等統計推論路線圖》：Canva 保留可編輯來源，PWA 使用既有 `math-bridge.svg` 離線快取路徑，並在附錄 B 提供 standalone SVG 放大入口。
  - 《數理經濟學》完成《最佳化與動態判斷地圖》：Canva 保留可編輯來源，PWA 沿用既有 `kkt-inequality.svg` 離線快取路徑，並在附錄 B 提供 standalone SVG 放大入口。
  - 數理經濟學正式 Actions：既有 855 checks、23 quantitative rechecks、10 high-risk logic gates 均通過；VP validator 40 checks、`visual_polish=passed`，部署後 artifact 重驗亦通過。
