...
- 正式書籍數：10 本。
- 最新成功 canonical Pages run：`30442682452`。
- 最新成功部署 source commit：`00cf9286eb7bbe3b2d8e2b6165cc163fd64a4e72`。
- 部署回條：`docs/deployment_receipt.json`；`status=success`、`book_count=10`、`progress_storage_changed=false`。
...
- 正式成功以 canonical workflow、Pages artifact 與 deployment receipt 為準；merge 成功本身不等於已部署。
...
7. 最新成功書庫：10 本，`2026.07.29-13`，run `30442682452`。
...
3. 建立 `docs/books/<book-id>/scope.md` 與 `status.md`，固定科目邊界。
4. 依標準本科課程製作教材、圖解、題庫、搜尋資料；必要時用可信官方／大學來源交叉核對。
5. 完成兩輪獨立 QA，特別重算數值題並設高風險負面 gate。
6. 發布前重新同步最新 main；確認其他平行教材是否已先正式發布。
7. 依 canonical workflow 加入同一 PWA；只有 deployment receipt 成功後才標記「已部署」。
8. 同步更新該書 status、QA report、README、deployment receipt 與本 checkpoint，任務才算收尾。
