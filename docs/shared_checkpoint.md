...
- 發布後獨立糾錯複核：正文與附錄修正 14 處、題庫同步修正 6 題；本次 1,333／1,333 項檢查與 52 個靜態路徑全數通過。
- 修正重點：補貼價格楔、規模經濟與規模報酬、正外部性、淨稅收、簡單存款乘數、AD 模型條件、淨稅收乘數、國際收支範圍及繁體／名詞一致性。
- 章節 ID、題目 ID 與儲存鍵未改變，既有閱讀進度與錯題紀錄保留。
- 範圍文件：`docs/books/economics/scope.md`
- QA 報告：`docs/books/economics/qa_report.md`
- 狀態：版本 `2026.07.27-2` 的內容與 QA 完成，待修正 PR 合併後正式部署。

## 部署流程

1. GitHub Actions 下載固定 ID 的正式網站部署包。
2. 先核對部署包 SHA-256，再用 Python ZIP 模組解開；部署包可含前置檔頭，不依賴桌面解壓工具。
3. 從 repo 內的修正程式套用《經濟學原理》版本 `2026.07.27-2`，同步重建搜尋索引、版本資訊與快取版本。
4. 驗證三本書的 manifest、題庫數量、章節檔、搜尋索引、經濟學圖解、修正詞句與 PWA 快取。
