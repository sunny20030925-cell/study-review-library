...
- 修正重點：補貼價格楔、規模經濟與規模報酬、正外部性、淨稅收、簡單存款乘數、AD 模型條件、淨稅收乘數、國際收支範圍及繁體／名詞一致性。
- 數值答案：代表性計算題重新驗算，未發現需要改答案數值的錯誤。
- 閱讀進度相容性：章節 ID、題目 ID、題數與儲存鍵未變；部署回條確認 `progress_storage_changed=false`。
- 範圍文件：`docs/books/economics/scope.md`
- QA 報告：`docs/books/economics/qa_report.md`
- 部署回條：`docs/deployment_receipt.json`
- PR：`#5`；合併 commit：`9839467820bb0bac261b33ce4fb7671dea60b25b`。
- 成功部署 workflow run：`30227689299`。
- 狀態：已合併至 `main` 並完成 GitHub Pages 正式部署。
...
3. 執行 repo 內的會計學第二版修正腳本，重建會計學搜尋索引並驗證 14 章、70 題與指定修正內容。
4. 執行 repo 內的經濟學第二版修正包，修正正文與題庫、重建 144 筆搜尋索引並提高內容及快取版本。
5. 驗證三本書的 manifest、題庫數量、章節檔、搜尋索引、圖解、指定修正詞句與 PWA 快取。
6. JavaScript 語法與全部正式檢查通過後，才上傳 Pages artifact。
7. Pages 部署成功後寫回 `docs/deployment_receipt.json`，失敗時不會產生成功回條。
8. 使用者不需要在平板上處理 ZIP、Git 或部署。
