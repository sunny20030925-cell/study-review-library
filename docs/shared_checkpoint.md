...
- 發布後獨立內容複核：重新檢查實際生成後正文、3 附錄與 90 題題庫；完成 12 組實質內容修正、6 題題庫精確化，題目 ID 與題數未變。
- 主要修正：公司法第 8 條第 3 項適用範圍、公開發行公司董事至少 5 人、董事會 3／7 日通知、利害關係董事表決排除、第 194 條 1 年門檻、閉鎖公司人數例外與勞務出資條件、短線交易其他股權性質證券，以及內線交易非股權公司債規則。
- 閱讀進度相容性：章節 ID、題目 ID 與題數未變；部署回條確認 `progress_storage_changed=false`。
- 部署驗證先移除 HTML 排版標記，再核對法律實質要件，並保留禁止誤述的負面 gate。
- 範圍文件：`docs/books/commercial-law/scope.md`
- QA 報告：`docs/books/commercial-law/qa_report.md`
- GitHub Pages 部署 run：`30394272983`。
...
4. 套用統計學 `deploy/patch_statistics_v2.py` 的發布後獨立二次校錯修正，再生成商事法內容，並套用 `deploy/patch-commercial-law-reaudit.py` 的發布後獨立內容複核修正。
...
7. 商事法額外驗證 18 章、3 附錄、90 題、111 筆搜尋索引、18 張 SVG，以及本輪新增的公司法／證券交易法高風險法律要件。
8. 商事法法律文字驗證先移除 HTML 排版標記與空白，再核對條號、比例、期間、人數及禁止誤述；不再要求單一逐字句型。
9. JavaScript、service worker 與全部正式檢查通過後，才上傳 Pages artifact。
10. Pages 部署成功後寫回 `docs/deployment_receipt.json`。
11. 最新五本書成功部署 run：`30394272983`；書庫內容版本：`2026.07.29-5`。
...
- 商事法發布後獨立內容複核、糾錯、6 題精確化與新版部署已完成；其章節 ID、題目 ID 與題數均未變。
