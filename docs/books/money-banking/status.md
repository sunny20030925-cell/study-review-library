...
- 狀態：內容與兩輪 QA 已完成，並已通過最新十一書（含財政學）Pages artifact 的 canonical tail 實跑；待正式 PR 合併與 Pages 部署。
...
- 最新十一書 artifact 實跑：`2026.07.29-14`／11 本／尾端 `public-finance` → `2026.07.29-15`／12 本／尾端 `money-banking`。
- 閱讀進度相容性：候選不修改既有 book ID、章節 ID、題目 ID 或儲存格式。

## 目前外部阻塞

- 財政學最新 canonical workflow 已產生並部署十一書 artifact，但在部署後寫回 `docs/shared_checkpoint.md` 時因 `microeconomics checkpoint section not found` 失敗。
- 貨銀整合本身未依賴該寫回步驟；正式發布前仍需以最新 `main` 為準，確認該共同 workflow 問題已修正或一併採用安全修正。
