...
1. 以最新正式 17 本 Pages artifact 為唯一發布底稿；尾端必須為 `civil-law-overview → investments`。
2. 數理經濟學只能接在正式 `investments` 後方，不跳過或覆蓋較新的正式 artifact。
3. 生成後執行第一輪結構／內容 QA 與第二輪獨立數值、公式、最佳化、比較靜態與穩定性重算。
4. 整合前後既有 17 本教材內容 hash 必須完全一致。
5. Pages artifact、Pages deployment、重新下載驗證與 deployment receipt 全部成功後，才改成「已部署」。

## 預定正式位置

- 正式順序尾端：`game-theory → civil-law-overview → investments → mathematical-economics`。
- 預定成為第 18 本教材。
