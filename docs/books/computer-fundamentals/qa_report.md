...
候選 workflow 會在當下最新已部署書庫 artifact 的副本上重新生成本書，並檢查：
...
8. 新書只能追加在當下正式書庫尾端；既有書籍逐檔 SHA-256 前後必須一致。
