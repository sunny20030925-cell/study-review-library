...
def main(site_root, expected_library='2026.07.29-12'):
 ...
 ck(len(lib['books'])==9 and [x['id'] for x in lib['books']][-1]==BOOK,'nine book order')
 ...
 print(f'MACROECONOMICS_V2_QA_OK checks={checks} books=9 library={expected_library} chapters=20 appendices=3 questions=100 search=143 figures=20 quantitative_rechecks={len(expected)} content_corrections=14 question_adjustments=7')

if __name__=='__main__':
 if len(sys.argv) not in (2,3): raise SystemExit('usage: qa_macroeconomics_v2.py SITE_ROOT [EXPECTED_LIBRARY]')
 main(sys.argv[1],sys.argv[2] if len(sys.argv)==3 else '2026.07.29-12')
