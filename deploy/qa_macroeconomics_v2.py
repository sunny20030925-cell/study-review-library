...
def main(site_root, expected_library='2026.07.29-13'):
 ...
 ck(len(ids)==10 and ids[-2:]==['macroeconomics','international-economics'],'ten book order')
 ...
 expected={
 'ch00-q03':('2 個百分點。','百分點'), 'ch00-q04':('約 4%。','名目成長'),
 'ch01-q04':('S=150，I=150。','S=Y-C-G'), 'ch01-q05':('NX=-80。','300-380'),
 'ch02-q01':('5%。','(126-120)/120'), 'ch02-q03':('120。','2.4/2.0'), 'ch02-q04':('約 4%。','i-π'),
 'ch03-q01':('約 5.26%。','50/950'), 'ch03-q04':('10%。','0.02/(0.02+0.18)'),
 'ch04-q02':('0.7。','份額'), 'ch04-q03':('約 3.9%。','0.3×4'),
 'ch06-q01':('私人儲蓄 250、政府儲蓄 -50、國民儲蓄 200。','Sp=1200'),
 'ch07-q01':('0.7。','700/1000'), 'ch07-q02':('NT$80。','0.8×100'),
 'ch09-q03':('10。','1/0.1'), 'ch10-q03':('約由 3% 升到 6%。','i≈r+π'),
 'ch11-q02':('NT$40 億。','1/(1-0.75)'), 'ch11-q03':('-3。','-0.75/(1-0.75)'),
 'ch16-q03':('約增加 2 個 GDP 百分點。','0.04-0.02'), 'ch18-q01':('80。','CA=S-I'),
 }
 for qid,(ans,exp_token) in expected.items():
  ck(qmap[qid]['answer']==ans,f'{qid} answer {qmap[qid]["answer"]!r}')
  ck(exp_token in qmap[qid]['explanation'],f'{qid} explanation token')
 ...
 required=[
  '穩態失業率為 \\(u^*=s/(s+f)\\)', 'NAIRU', '兩者概念並非完全同義',
  '為使投資提高到與較高儲蓄相等，需要較低利率', '實質工資 \\(W/P\\) 下降',
  'Taylor principle', '不同教材也會把當期通膨本身另外寫進規則',
  '經常帳 CA 包含淨出口、國外淨要素所得與淨經常移轉', '\\(CA=S-I\\)', '\\(CA\\approx NX\\)',
  '\\(\\Delta e/e\\approx\\pi_{TW}-\\pi_{US}\\)', '本國貨幣傾向貶值',
  '均衡產出在基準模型中不變', '財政擴張被完全抵銷', '獨立貨幣政策對產出沒有持久效果',
 ]
 for token in required: ck(token in text,f'missing corrected token {token}')
 forbidden=[
  '為使儲蓄等於較低投資，需要較低利率', '企業的實質收益可能上升而增加產出',
  '抵銷部分甚至大部分擴張', '自然失業率是通膨穩定時與勞動市場制度相容的失業率',
 ]
 for token in forbidden: ck(token not in text,f'forbidden stale text {token}')
 ck(qmap['ch00-q03']['answer']!='約 2%。','percentage point stale answer')
 ck('職缺率 2%、失業率 18%' not in qmap['ch03-q04']['question'],'bad flow rates removed')

 for ch in m['chapters']:
  p=root/ch['file']; ck(p.is_file() and p.stat().st_size>500,f'file {ch["id"]}')
  html=p.read_text(encoding='utf-8'); ck('\f' not in html and '\t' not in html,f'controls {ch["id"]}'); ck('<script' not in html.lower(),f'inline script {ch["id"]}')
 ...
 for token in ['CA=S-I','NAIRU','Taylor principle','完全抵銷']:
 ...
 print(f'MACROECONOMICS_V2_QA_OK checks={checks} books=10 library={expected_library} chapters=20 appendices=3 questions=100 search=143 figures=20 quantitative_rechecks={len(expected)} content_corrections=14 question_adjustments=7')

if __name__=='__main__':
 if len(sys.argv) not in (2,3): raise SystemExit('usage: qa_macroeconomics_v2.py SITE_ROOT [EXPECTED_LIBRARY]')
 main(sys.argv[1],sys.argv[2] if len(sys.argv)==3 else '2026.07.29-13')
