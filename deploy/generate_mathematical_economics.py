...
def tex(s: str) -> str: return s.replace("\\'", "'")
...
def chapter_html(ch):
    ...
    formulas=''.join(f'<div class="formula-card"><p class="math display">\\[{html.escape(tex(a))}\\]</p><p>{html.escape(b)}</p></div>' for a,b in ch['formulas'])
...
def render_svg(ch):
    ...
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 835 390" role="img" aria-labelledby="title desc"><title id="title">{xml_escape(title)}</title><desc id="desc">{xml_escape(' → '.join(labels))}</desc><rect width="835" height="390" rx="26" fill="#fff"/><text x="417" y="72" text-anchor="middle" font-size="30" font-weight="700" font-family="sans-serif" fill="#4c1d95">{xml_escape(title)}</text>{''.join(arrows)}{''.join(boxes)}<text x="417" y="318" text-anchor="middle" font-size="17" font-family="sans-serif" fill="#475569">先辨認數學物件，再運算；最後檢查條件、符號與可行域。</text></svg>\n'''
...
def appendix_a():
    ...
    for ch in CHAPTERS:
        for formula,note in ch['formulas']:
            rows.append(f'<tr><td>{html.escape(ch["title"])}</td><td class="math">\\({html.escape(tex(formula))}\\)</td><td>{html.escape(note)}</td></tr>')
    return '<h1>附錄 A　核心公式速查</h1><p class="lead">考前用這份表確認公式、符號與成立條件；最危險的失分通常不是算術，而是把必要條件當充分條件或漏掉可行域。</p><table><thead><tr><th>章節</th><th>公式</th><th>條件／用途</th></tr></thead><tbody>'+''.join(rows)+'</tbody></table>'


def appendix_b():
    return '''<h1>附錄 B　數理經濟學解題路線</h1><p class="lead">陌生題先分類，再選工具。不要一看到符號就微分，也不要一看到限制就不分等式／不等式直接套 Lagrange。</p><h2>方程與矩陣</h2><ol><li>數未知數與獨立方程。</li><li>線性系統先寫 Ax=b。</li><li>需要唯一解時檢查 determinant／rank／可逆性。</li><li>最後代回全部方程。</li></ol><h2>微分與比較靜態</h2><ol><li>固定基準點、內生變數與外生參數。</li><li>顯函數直接微分；隱式均衡先寫 F=0 再全微分。</li><li>比較靜態不是時間路徑。</li><li>小變動近似要標示近似號。</li></ol><h2>最佳化</h2><ol><li>先寫可行域。</li><li>無限制：FOC→SOC／凹凸性→端點。</li><li>等式限制：Lagrangian＋限制式。</li><li>不等式：KKT 四塊一起檢查；不能漏互補鬆弛。</li></ol><h2>動態</h2><ol><li>差分方程令 x_{t+1}=x_t 求穩態。</li><li>微分方程令 ẋ=0 求穩態。</li><li>再看特徵根絕對值或 F′(x*) 的符號。</li><li>穩態、穩定與最適是三個不同概念。</li></ol>'''


def appendix_c():
    terms=[('Variable','變數'),('Parameter','參數'),('Function','函數'),('Equilibrium','均衡'),('Vector','向量'),('Matrix','矩陣'),('Determinant','行列式'),('Inverse Matrix','反矩陣'),('Eigenvalue','特徵值'),('Quadratic Form','二次型'),('Limit','極限'),('Derivative','導數'),('Taylor Approximation','Taylor 近似'),('Partial Derivative','偏導數'),('Total Differential','全微分'),('Gradient','梯度'),('Jacobian','Jacobian 矩陣'),('Hessian','Hessian 矩陣'),('Comparative Statics','比較靜態'),('Implicit Function Theorem','隱函數定理'),('Elasticity','彈性'),('Lagrangian','Lagrangian'),('Lagrange Multiplier','Lagrange 乘數'),('KKT Conditions','KKT 條件'),('Complementary Slackness','互補鬆弛'),('Envelope Theorem','包絡定理'),('Value Function','值函數'),('Homogeneous Function','齊次函數'),('Euler Theorem','Euler 定理'),('Integral','積分'),('Difference Equation','差分方程'),('Differential Equation','微分方程'),('Steady State','穩態'),('Stability','穩定性')]
...
def search_entries():
    entries=[]
    fields=[('正式定義與核心概念',lambda c:' '.join(f'{a}：{b}' for a,b in c['definitions'])),('核心公式與成立條件',lambda c:' '.join(f'{tex(a)}；{b}' for a,b in c['formulas'])),('完整標準例題',lambda c:' '.join(c['example'])),('常見錯誤',lambda c:' '.join(c['traps'])),('考試判斷方法',lambda c:' '.join(c['exam'])),('理解檢查',lambda c:' '.join(c['checks']))]
    for ch in CHAPTERS:
...
def append_sw_assets(sw,anchor,paths):
    if paths[0] in sw: return sw
    idx=sw.find(f'./books/{anchor}/manifest.json')
    if idx<0: raise AssertionError(f'cache anchor missing: {anchor}')
    end=sw.find('];',idx)
    if end<0: raise AssertionError('asset array end missing')
...
def main(site_root):
    ...
    manifest={'id':BOOK,'title':TITLE,'subtitle':SUBTITLE,'version':VERSION,'cover':COVER,'accent':ACCENT,'features':{'reader':True,'quiz':True,'formula':True},'updatedAt':UPDATED_AT,'chapters':meta,'releaseNotes':[{'version':VERSION,'date':UPDATED_AT,'title':'新增一般大學數理經濟學教材','changes':['新增 20 章正文與 3 份附錄','新增 100 題題庫、150 筆搜尋索引與 20 張自製 SVG','核心涵蓋矩陣、微積分、比較靜態、Hessian、Lagrange、KKT、積分與一階動態'],'progressImpact':'新增獨立書籍，不改動既有書籍章節 ID、題目 ID、錯題紀錄或閱讀進度。'}]}
    (root/'manifest.json').write_text(jdump(manifest),encoding='utf-8')
    qitems=[]
    for src in build_questions():
        slot=src['id'].rsplit('-',1)[-1]; qitems.append({'id':src['id'],'bookId':BOOK,'chapterId':src['chapterId'],'chapterTitle':titles[src['chapterId']],'topic':next(c['title'] for c in CHAPTERS if c['id']==src['chapterId']),'difficulty':DIFFICULTY[slot],'question':src['question'],'answer':src['answer'],'explanation':src['explanation'],'source':'本書自編標準題型'})
...
if __name__=='__main__':
    if len(sys.argv)!=2: raise SystemExit('usage: generate_mathematical_economics.py SITE_ROOT')
    main(sys.argv[1])
