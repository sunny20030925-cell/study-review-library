...
def main(site_root:str,expected_library:str)->int:
 ...
 ck(m['version']==q['version']==VERSION,'book versions'); ck(lib['version']==expected_library,'library version'); ids=[b['id'] for b in lib['books']]; ck(BOOK in ids and ids.count(BOOK)==1,'book order/identity'); ck(len([x for x in m['chapters'] if x['kind']=='chapter'])==20,'chapters'); ck(len([x for x in m['chapters'] if x['kind']=='appendix'])==3,'appendices'); ck(q['count']==len(q['items'])==100,'questions'); ck(len(s['entries'])==150,'search')
