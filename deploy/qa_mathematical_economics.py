...
def main(site_root: str, expected_library: str) -> None:
    ...
    ck(len(ids)==14 and ids[-3:]==['money-banking','computer-fundamentals',BOOK],'fourteen-book canonical tail')
    ...
    for p in figs:
        x=p.read_text(encoding='utf-8'); ck('<title' in x and '<desc' in x and 'viewBox=' in x,f'accessibility svg {p.name}'); ck('href="http' not in x and "href='http" not in x,f'no remote svg {p.name}')
    ...
    calc={
      'ch00':120-3*20,'ch01':5+4*3,'ch02_p':(100-20)/(2+2),'ch02_q':100-2*20,'det':2*1-1*1,
      'taylor':100+20*0.1,'derivative':3*2**2,'fx':2*2+3,'fy':2,'dz':7*0.1+2*(-0.2),
      'implicit':1/(2*2),'implicit_change':(1/4)*0.4,'opt_q':20/2,'opt_value':20*10-10**2,
      'multi_x':10/2,'multi_y':8/2,'lagrange_x':10/2,'kkt_boundary':3,'cost3':10+3**2+4*3,
      'compound':100*(1.05**2),'continuous':math.log(1.05),'difference_ss':10/(1-0.5),'differential_ss':6/0.3,
    }
    ...
    print(f'MATHEMATICAL_ECONOMICS_QA_OK total_checks={checks} books=14 library={expected_library}')
