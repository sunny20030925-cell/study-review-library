...
def main(sr,ev):
 ...
 ch=(r/'chapters/ch00.html').read_text();ap=(r/'chapters/appendix-b.html').read_text();svg=(s/A).read_text();ck(ch.count(M)==1 and ap.count(M)==1,'mark');ck(A in ch and A in ap,'asset');ck('target="_blank"' in ch and 'target="_blank"' in ap,'zoom');vp=m['visualPolish'];ck(vp['status']=='passed' and vp['asset']==A,'vp');ck(vp['canvaDesignId']==C and vp['canvaEditUrl']==U,'canva');ck(vp['tabletReadable'] and vp['zoomable'],'flags');ck(vp['offlineCachedViaExistingAssetPath'] is False and vp['offlineCachedViaServiceWorker'] is True,'cache mode')
