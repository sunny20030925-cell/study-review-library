...
HTML_REPLACEMENTS = {
    ...
    "appendix-a": [
        ("\\(R_i=E[R_i]+\\beta_{i1}F_1+\\cdots+\\beta_{ik}F_k+e_i\\)</td><td>一般多因子報酬表示。</td>", "\\(R_i=E[R_i]+\\beta_{i1}F_1+\\cdots+\\beta_{ik}F_k+e_i\\)</td><td>此寫法要求 F_j 為零均值的因子 surprise／去均值因子，且 E[e_i]=0。</td>"),
        ("\\(ROE=\\frac{NI}{Sales}\\times\\frac{Sales}{Assets}\\times\\frac{Assets}{Equity}\\)</td><td>三段 DuPont 分解。</td>", "\\(ROE=\\frac{NI}{Sales}\\times\\frac{Sales}{Average\\ Assets}\\times\\frac{Average\\ Assets}{Average\\ Equity}\\)</td><td>三段 DuPont；期間流量通常與平均資產／平均權益存量配對。</td>"),
        ("\\(F_0\\approx S_0(1+r)^T\\)</td><td>無收益標的簡化成本持有關係；實務需調整股利、收益、儲存成本等。</td>", "\\(F_0=S_0(1+r)^T\\)</td><td>無收益、無其他 carry、可按 r 融資／投資且無套利時的離散複利等式。</td>"),
        ("\\(R_{home}\\approx(1+R_{foreign})(1+R_{FX})-1\\)</td><td>外幣資產本幣報酬的乘法關係；R_FX 定義需先固定匯率方向。</td>", "\\(1+R_{home}=(1+R_{foreign})(1+R_{FX})\\)</td><td>精確乘法關係；本章固定 S 為本幣／外幣，R_FX=S_1/S_0-1。</td>"),
        ("\\(IR=\\frac{R_P-R_B}{\\sigma(R_P-R_B)}\\)</td><td>資訊比率；分子通常用平均主動報酬。</td>", "\\(IR=\\frac{\\overline{R_P-R_B}}{\\sigma(R_P-R_B)}\\)</td><td>平均主動報酬除以 tracking error；期間／年化口徑需一致。</td>"),
    ],
    "appendix-c": [
        ("<tr><td>Exchange-Traded Fund</td><td>指數股票型基金 ETF</td></tr>", "<tr><td>Exchange-Traded Fund</td><td>交易所交易基金 ETF（臺灣被動式 ETF 法規中文名稱為「指數股票型基金」；另有主動式 ETF）</td></tr>"),
    ],
}

QUESTION_UPDATES = {
    "ch09-q03": {"explanation": "APT 可包含多個共同因子。若報酬生成式把截距直接寫成 E[R_i]，因子通常以零均值的 surprise／去均值變動表示，殘差也取零均值；這和「哪些因子具有風險溢酬」是不同問題。"},
    "ch13-q01": {"explanation": "題目已直接給淨利率、資產週轉率與權益乘數，因此 ROE=0.05×2×1.5=0.15。若是自行由財報計算後兩項比率，期間銷售通常要和平均資產配對，權益乘數也常以平均資產／平均權益計算。"},
    "ch15-q05": {"explanation": "時間經過、殖利率與現金流變化都會改變 duration，通常需要再平衡。單一負債常同時關注資產價值與 Macaulay duration；多筆負債還會比較 market value、money duration／BPV，並留意凸性與曲線形狀風險。"},
    "ch19-q04": {"explanation": "ETF 是在交易所交易的基金架構，不等於一定被動追蹤指數，也不保證充分分散。主動式、產業型、主題型或高度集中 ETF 都可能集中於少數風險來源。"},
    "ch20-q04": {"explanation": "還可能包含政治、稅務、流動性與市場制度差異。匯率效果要先固定報價方向；若 S 是本幣／外幣，則 1+R_home=(1+R_foreign)(1+R_FX)。"},
    "ch21-q04": {"explanation": "IR=平均主動報酬/追蹤誤差；tracking error 是 RP−RB 的標準差，分子與分母需使用一致的期間／年化口徑。"},
}
