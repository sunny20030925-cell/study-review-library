...
def main(site_root: str, pre_library_path: str) -> None:
    ...
    pre_ids=[x['id'] for x in pre['books']]
    post_ids=[x['id'] for x in post['books']]
    if BOOK_ID in pre_ids: raise AssertionError('book already existed before generation')
    if post_ids != pre_ids+[BOOK_ID]: raise AssertionError(f'book order drift: {pre_ids} -> {post_ids}')
    final=next_library_version(pre['version'])
    post['version']=final
    libp.write_text(json.dumps(post,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
...
if __name__=='__main__':
    if len(sys.argv)!=3: raise SystemExit('usage: finalize_international_economics_library.py SITE_ROOT PRE_LIBRARY_JSON')
