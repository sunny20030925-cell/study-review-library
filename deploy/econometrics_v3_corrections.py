...
def build_v3(chapters, questions):
    ...
    ch['intuition'].append('若 treatment effect 因人而異，二元工具變數的 Wald／2SLS 比率不能無條件叫作整體 ATE 或 ATT。在標準二元 IV 架構下，若工具外生、滿足 exclusion、確實改變 treatment，並採用 monotonicity（沒有 defiers）等條件，最典型的因果 estimand 是被工具推動而改變 treatment 狀態之 compliers 的 Local Average Treatment Effect（LATE／CACE）。')
    ...
    ch['formulas'].append((r'LATE=E[Y(1)-Y(0)\mid D(1)>D(0)]', '在二元 treatment／instrument 且工具外生、relevance、exclusion、monotonicity 等標準條件下，Wald 比率可解讀為 compliers 的平均因果效果，而非自動等於全母體 ATE／ATT。'))
    ...
    ch['example'][2] = '若 treatment 組有人未參加、control 組有人自行參加，把大家依「實際參加」重分組就不再保證隨機。此時仍先報 ITT；若以原始 assignment 作為實際 treatment 的工具，在 relevance、exclusion、monotonicity 等相應條件下，可進一步識別 compliers 的 LATE／CACE。這不是把所有實際接受 treatment 者直接當成一個仍然隨機的群組，也不自動等於 ATT／TOT。'
    ch['definitions'].append(('LATE under noncompliance', '隨機 assignment 作 IV 且相應工具變數假設成立時，實際 treatment 的典型局部因果 estimand 是會因 assignment 改變 treatment 狀態之 compliers 的平均效果。'))
    ...
    ch['intuition'].append('另一個常被省略的條件是 no anticipation：被標成「處置前」的期間不應已因預期政策而改變結果。若企業或個人在政策正式生效前就提前調整，這些 pre-period observations 已受 treatment timing 影響，標準 pre／post DiD 與 event-study 基準期就要重新界定或明確建模 anticipation window。')
    ch['definitions'].append(('No anticipation（無預期／無提前反應）', '在標準 DiD 設定中，處置正式發生前的潛在結果不因未來 treatment 而先行改變；若有已知提前反應，需調整 treatment timing 或明確處理 anticipation period。'))
    ...
    ch['intuition'].append('Fuzzy RDD 的 treatment probability 只在 cutoff 跳動，因此本質上可把「跨過 cutoff」視為局部工具變數。若 treatment effects 異質，除了 cutoff 附近的連續性與 first-stage jump，還需要相應 exclusion／monotonicity 等條件；此時局部 Wald ratio 的標準解讀是 cutoff 處 compliers 的 LATE，而不是整體 ATE。')
    ch['definitions'].append(('Local complier in fuzzy RDD', '在 cutoff 附近，因 crossing／eligibility 狀態改變而改變實際 treatment 的單位；fuzzy RDD 的標準局部因果效果針對這群 compliers。'))
    for i, (formula, note) in enumerate(ch['formulas']):
        if r'\tau_{FRD}' in formula:
            ch['formulas'][i] = (formula, 'Fuzzy RDD 的局部 Wald ratio；在 cutoff 連續性、first-stage jump、exclusion、monotonicity 等標準條件下，因果解讀是 cutoff 處 local compliers 的 LATE，而非無條件的全體 ATE。')
            break
    else:
    ...
    ch['example'][1] = '若一個複雜模型 training RMSE=0.5、validation RMSE=3，而簡單模型 training RMSE=1.2、validation RMSE=1.8，應在尚未打開 final test set 前依 validation／cross-validation 結果選擇後者。模型與超參數固定後，才用保留的 test set 做一次最後樣本外評估。'
    ch['example'].append('若反覆比較多個模型的 test RMSE，再挑 test 表現最好的那一個，test 已參與 model selection；之後同一 test RMSE 會對真正泛化能力過度樂觀。此時需要新的未使用資料，或重新建立嚴格的 validation／test 流程。')
...
def appendix_c_v3(base_html: str) -> str:
    rows = ('<tr><td>Sample Average Treatment Effect (SATE)</td><td>樣本平均處置效果</td></tr>'
