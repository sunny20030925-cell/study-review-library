...
def main(site_root: str) -> None:
    ...
    corrections += patch_html(book / "chapters/ch10.html", [
        (
            "最小成本寫成 \\(C(w,r,q)=wL^c+rK^c\\)。它對投入價格一次齊次且凹，對產量通常非遞減。短期成本因某些投入固定；長期成本允許所有投入調整。",
            "在目標產量可行、投入價格非負且最小值存在等標準條件下，最小成本寫成 \\(C(w,r,q)=wL^c+rK^c\\)。它對投入價格一次齊次且凹，對產量非遞減。短期成本因某些投入固定；長期成本允許所有投入調整。",
            "cost-function-regularity",
        ),
    ])
    ...
    corrections += patch_html(book / "chapters/ch17.html", [
        ...
        (
            "因此在 \\(n\\) 個市場中，只要 \\(n-1\\) 個市場出清，且最後一個市場的價格為正，最後一個市場也會出清。",
            "在上述 Walras 法則成立下，因此在 \\(n\\) 個市場中，只要 \\(n-1\\) 個市場出清，且最後一個市場的價格為正，最後一個市場也會出清。",
            "last-market-clearing-condition",
        ),
    ])
    ...
    corrections += patch_html(book / "chapters/appendix-b.html", [
        ...
        (
            "成本最小：\\(MRTS=w/r\\)。利潤最大：競爭企業 \\(p=MC\\)；投入條件 \\(pMP_i=w_i\\)。Hotelling：\\(\\pi_p=q^*\\)、\\(\\pi_{w_i}=-x_i^*\\)。",
            "平滑內點的成本最小：\\(MRTS=w/r\\)。競爭企業的內點利潤最大：\\(p=MC\\)；投入條件 \\(pMP_i=w_i\\)。Hotelling 在利潤函數可微且最適解適用包絡定理時：\\(\\pi_p=q^*\\)、\\(\\pi_{w_i}=-x_i^*\\)。",
            "producer-formula-conditions",
        ),
        (
            "獨占：\\(MR=MC\\)。Lerner：\\((P-MC)/P=1/|\\varepsilon_D|\\)。交換效率：\\(MRS_A=MRS_B\\)。Walras：\\(p\\cdot z(p)=0\\)。公共財：\\(\\sum_i MRS_i=MRT\\)。",
            "獨占內點：\\(MR=MC\\)。Lerner（可微需求且彈性有限）：\\((P-MC)/P=1/|\\varepsilon_D|\\)。交換效率（平滑內點）：\\(MRS_A=MRS_B\\)。Walras（個別預算等式成立）：\\(p\\cdot z(p)=0\\)。公共財（可微內點）：\\(\\sum_i MRS_i=MRT\\)。",
            "market-welfare-formula-conditions",
        ),
    ])

    content_corrections = corrections
    ...
    search_repls = [
        ...
        ("最小成本寫成 \\(C(w,r,q)=wL^c+rK^c\\)。它對投入價格一次齊次且凹，對產量通常非遞減。", "在目標產量可行、投入價格非負且最小值存在等標準條件下，最小成本寫成 \\(C(w,r,q)=wL^c+rK^c\\)。它對投入價格一次齊次且凹，對產量非遞減。"),
        ...
        ("成本最小：\\(MRTS=w/r\\)。利潤最大：競爭企業 \\(p=MC\\)；投入條件 \\(pMP_i=w_i\\)。Hotelling：", "平滑內點的成本最小：\\(MRTS=w/r\\)。競爭企業的內點利潤最大：\\(p=MC\\)；投入條件 \\(pMP_i=w_i\\)。Hotelling 在利潤函數可微且最適解適用包絡定理時："),
        ("獨占：\\(MR=MC\\)。Lerner：", "獨占內點：\\(MR=MC\\)。Lerner（可微需求且彈性有限）："),
    ]
    ...
    question_adjustments = corrections - content_corrections
    if content_corrections != 16 or question_adjustments != 4:
        raise AssertionError(f"unexpected correction counts content={content_corrections} questions={question_adjustments}")
    print(f"MICRO_V2_PATCH_OK corrections={content_corrections} question_adjustments={question_adjustments} version={NEW_VERSION}")
