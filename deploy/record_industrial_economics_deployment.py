...
def main(site_root: str) -> None:
    ...
    if not ids or ids[-1] != BOOK or ids.count(BOOK) != 1:
        raise AssertionError(f'industrial economics not unique tail: {ids}')
    ...
    if manifest['version'] != BOOK_VERSION or questions['version'] != BOOK_VERSION:
        raise AssertionError('content version drift')
    if (
        len(manifest['chapters']) != 23
        or questions['count'] != 100
        or len(search['entries']) != 150
        or len(list((site / 'assets/industrial-economics-svg').glob('*.svg'))) != 20
    ):
        raise AssertionError('artifact counts drift')
    ...
    receipt.update({
        ...
        'industrial_economics_round1_check_count': int(os.environ.get('INDUSTRIAL_QA1_CHECKS', '0') or 0),
        'industrial_economics_round2_check_count': int(os.environ.get('INDUSTRIAL_QA2_CHECKS', '0') or 0),
        'industrial_economics_quantitative_recheck_count': 17,
        'industrial_economics_high_risk_concept_gate_count': 18,
        'industrial_economics_two_round_qa': 'passed',
...
- 兩輪 QA 通過；17 個量化節點獨立重算、18 組高風險模型／條件複核。
...
'''
    ...
    readme = replace_once(
        readme,
        r'目前內容版本：`[^`]+`',
        f'目前內容版本：`{library["version"]}`',
        'README version',
    )
    book_line = (
        f'- 《產業經濟學》：一般大學產業組織核心，20 章、3 附錄、100 題、'
        f'150 筆搜尋索引與 20 張圖解；內容版本 `{BOOK_VERSION}`。'
    )
    ...
    checkpoint = replace_once(
        checkpoint,
        r'- Pages 狀態：.*$',
        f'- Pages 狀態：正式 artifact 上傳、Pages deployment 與下載後 artifact recheck 均成功；`{deployed_at}`。',
        'checkpoint pages status',
    )
    checkpoint = replace_once(
        checkpoint,
        r'- 部署回條：.*$',
        f'- 部署回條：`docs/deployment_receipt.json`；`status=success`、`book_count={count}`、`library_version={library["version"]}`、`progress_storage_changed=false`。',
        'checkpoint receipt',
    )
    checkpoint = replace_once(
        checkpoint,
        r'- 實際下載正式 Pages artifact 後再次核對：.*$',
        f'- 實際下載正式 Pages artifact 後再次核對：{count} 本 registry；產業經濟學 23 份章節／附錄 HTML、100 題、150 搜尋、20 SVG 均存在；下載檔 SHA-256 與 GitHub artifact digest 一致。',
        'checkpoint artifact recheck',
    )
    checkpoint = replace_once(
        checkpoint,
        r'- workflow overall conclusion：.*$',
        '- workflow overall conclusion：`success`；post-deploy recorder 採結構化 Book ID／receipt 更新。',
        'checkpoint workflow conclusion',
    )

    section = f'''### {count}. 產業經濟學
- Book ID：`{BOOK}`
- 正式內容版本：`{BOOK_VERSION}`
...
'''
    ...
    checkpoint = re.sub(
        r'- canonical post-deploy recorder .*$',
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        raise SystemExit('usage: record_industrial_economics_deployment.py SITE_ROOT')
    main(sys.argv[1])
