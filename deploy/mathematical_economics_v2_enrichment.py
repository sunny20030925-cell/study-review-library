...
def enriched_chapters():
    ...
    ch['formulas'].extend([
        ...
        (r'\operatorname{rank}(A)=\operatorname{rank}([A\mid b])', 'Ax=b 有解的相容性條件；若共同 rank 又等於未知數個數 n，則解唯一；若共同 rank<n，則有自由變數而存在無限多解。'),
    ])
    ...
    ch['exam'] = ['先數未知數與獨立方程。', 'D≠0 可直接判唯一解；D=0 時比較 rank(A) 與 rank([A|b])：不同則無解，相同再看共同 rank 是否等於未知數個數。', '解完代回每一式，確認同時成立並符合經濟可行域。']
    ...
    ch = by_id['ch12']
    ch['definitions'].append(('bordered Hessian', '把等式限制的梯度「鑲邊」在 Lagrangian Hessian 外形成的矩陣；它是檢查受限制二階條件的常用行列式工具，但 max/min 的符號規則會隨限制數、變數數與符號慣例改變。'))
    ch['formulas'].append((
        r'B=\begin{pmatrix}0&g_x&g_y\\g_x&\mathcal L_{xx}&\mathcal L_{xy}\\g_y&\mathcal L_{yx}&\mathcal L_{yy}\end{pmatrix}',
        '二變數、單一等式限制的 bordered Hessian 結構。考試若不確定行列式正負規則，回到「所有非零可行切方向 d 上檢查 d^T∇²_{xx}L d」最不容易混淆。',
    ))
    ch['exam'] = ['先統一把限制寫成固定格式並解 FOC。', '二階判斷先想「只沿可行切方向看 Lagrangian 曲率」；bordered Hessian 是這件事的行列式捷徑，不是另一套經濟理論。', '若題目問 λ 經濟意義，先說清楚 Lagrangian 的符號與限制右側參數的進入方式。']
