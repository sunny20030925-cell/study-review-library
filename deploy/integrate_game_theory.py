...
EXPECTED_PREVIOUS_BOOK = 'computer-fundamentals'
EXPECTED_EXISTING_BOOK_COUNT = 14
...
def integrate(site_root: str, expected_before: str) -> str:
    ...
    if len(pre_ids) != EXPECTED_EXISTING_BOOK_COUNT or pre_ids[-1] != EXPECTED_PREVIOUS_BOOK:
        raise AssertionError(f'game theory requires current 14-book computer-fundamentals tail, got {pre_ids}')
    ...
    with tempfile.TemporaryDirectory(prefix='game-theory-') as temp_dir:
        ...
        if generated['version'] != expected_before:
            raise AssertionError('game theory generator must not pre-increment shared library version')
