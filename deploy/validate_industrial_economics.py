...
def main(site_root:str,expected_library:str)->int:
 ...
 ids=[b['id'] for b in lib['books']]; ck(len(ids)>=13 and BOOK in ids and ('industry-trade' not in ids or ids.index(BOOK)<ids.index('industry-trade')),'industrial economics registry order'); ck(ids.count(BOOK)==1,'single industrial economics registry entry')
