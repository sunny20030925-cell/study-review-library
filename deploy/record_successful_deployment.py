...
def main(site_root: str) -> None:
    ...
    receipt[f'{last_id.replace("-","_")}_version']=manifest.get('version'); receipt[f'{last_id.replace("-","_")}_chapter_count']=count_kind(manifest,'chapter'); receipt[f'{last_id.replace("-","_")}_appendix_count']=count_kind(manifest,'appendix'); receipt[f'{last_id.replace("-","_")}_question_count']=questions.get('count',len(questions.get('items',[]))); receipt[f'{last_id.replace("-","_")}_search_count']=len(search.get('entries',[])); receipt_path.write_text(jdump(receipt),encoding='utf-8')
